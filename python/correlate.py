#--------------------------------------------------------------
# Code from Chongxi Lai
# (https://github.com/chongxi/spiketag/blob/master/spiketag/core/correlate.py)
#--------------------------------------------------------------
import numpy as np
...
def CCG(spk_time, spk_id, window_bins=50, bin_size=1, fs=25000.0,
    ...
    '''
        ...
        merge_zero: merge 0 ms bin into (0, 1] ms bin
        ...
        * Please note that bin includes left edge for negative bins, and right
        edge for positive bins.

        For example,
        -25 ms <= ccg[i, j][0] < -24 ms
        -24 ms <= ccg[i, j][1] < -23 ms
        ...
        -1 ms <= ccg[i, j][24] < 0 ms
        ccg[i, j][25] == 0 ms
        0 ms < ccg[i, j][26] <= 1 ms
        ...
        24 ms < ccg[i, j][50] <= 25 ms

        * Since width of 0 ms bin is tiny (should be exact zero), its peak will
        be lower than other bins. For visualization, one can merge the
        zero ms bin (ccg[i, j][25]) by using 'merge_zero=True'.
...
def correlate(spike_time, membership, cluster_ids, fs=25e3, window_bins=50,
        bin_size=1, merge_zero=True):
    '''
    ...
    return _symmetrize_ccg(half_ccg, merge_zero=merge_zero)
...
def _symmetrize_ccg(half_ccg, merge_zero=True):
    ...
    if merge_zero:
        half_ccg[..., 1] += half_ccg[..., 0]
        return np.dstack((sym, half_ccg[..., 1:]))
    else:
        return np.dstack((sym, half_ccg))
